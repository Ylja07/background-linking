...
def get_docids(topics: str, candidates: str, topics_only: bool) -> list:
    ...
    topic_docids = utils.read_topic_ids_from_file(
        f"../resources/topics-and-qrels/{topics}"
    )
    ...
    if topics_only:
        ...
        return [
            docid for qid in qid_docids.keys() for docid in qid_docids[qid]
        ] + topic_docids
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--index",
        dest="index",
        default="lucene-index.core18.pos+docvectors+rawdocs_all_v3",
        help="Document index",
    )
    parser.add_argument(
        "--name",
    ...
    parser.add_argument(
        "--candidates",
        dest="candidates",
        default="run.backgroundlinking19.bm25+rm3.topics.backgroundlinking19.txt",
    ...
    parser.add_argument(
        "-n", dest="n", default=100, type=int, help="Number of tfidf terms to extract"
    )
    parser.add_argument(
        "--cut",
        dest="cut",
    ...
    for docid in tqdm(all_docids[: args.cut]):
        ...
        # Format tfidf terms
        terms = [
            f"{term};;;{locations};;;{tfidf_terms[term]}"
        ...
        entities = [
            f"{entity};;;{locations};;;{len(locations)};;;{entity_type_dict[entity]}"
            for entity, locations in entity_loc_dict.items()
        ]

        # Insert into sql database.
        cursor.execute(
            "INSERT INTO entities (docid, entity_ids, tfidf_terms) VALUES (?,?,?)",
            (docid, "\n".join(entities), "\n".join(terms)),
        )
        conn.commit()
